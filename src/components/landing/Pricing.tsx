...
export function Pricing() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Fallback: nunca ocultar las tarjetas (evita que a veces "falte" un plan)
      gsap.set(cards, { opacity: 1 });

      // Animación sin tocar opacity (solo desplazamiento) para no dejar nada invisible por error
      gsap.fromTo(
        cards,
        { y: 40 },
        {
